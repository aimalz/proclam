\section{Data\footnote{Data in this paper refers to classification results, not lightcurves; no \plasticc\ lightcurves were simulated, viewed, or classified in the preparation of this paper.}}
\label{sec:data}

\aim{[Rahul: Would it help to avoid confusion if we change `toy data` be changed to toy/Mock classification results? Secondly, I am worried about the usage of the word `systematics` ... I admit I hate that word in general because too many different things get clumped in that word, but in this case, I wonder if calling systematics 'properties of classifiers' would be more descriptive]}
...
\aim{[Maybe use the same C symbol from equation 1 here. How are these probabilities constructed?  number of objects of m' classified as m globally]}
...
\aim{[Rahul: Figure caption should say confusion matrix! It might be good to have a sequential color bar, It would seem that the green in Perfect should correspond to lower values than the green in agnostic, and I think it is true, but hard to see in the color bar because of the colormap choice, Also, I think agnostic is what the text calls uncertain, Accurate/Perfect. It might be good to match the subsections to the plots. I am also not sure if the description of the subsumed classifiers is correct, it seems like a one to many sense is missing in the description. Finally, why is the tunnel vision correlation vertical?]}
\begin{figure*}
	\begin{center}
