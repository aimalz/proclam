...
Notions of accuracy, purity, completeness, and other terms endemic in science are examples of traditional metrics appropriate to classification point estimates. \aim{[We need to be consistent in the use of either `point estimates` or `Deterministic Classification` throughout the paper]}
...
\aim{I think I addressed the issue of SNPhotCC metrics above, but missed the point on the Narayan+18 reference. I was trying to change this language because `incompatibility` is perhaps not the right word.}
...
A possible 
\aim{[Rahul: I did not understand the following point, perhaps it could be rephrased]}
...
\aim{[Rahul: looks like the second part of the two-fold problem is missing]}
...
\aim{[Rahul: Maybe there should be a point that they will actually also under perform in reality ie. LSST]}
...
\aim{[Rahul: I am not sure why the statement below is necessary: why should we not treat a deterministic classifier as giving probabilities 1 for the identified class and 0 for all others? Is this is because the logloss metric is not defined for these values?]}
...
\aim{[Rahul: But this is not a part of the metric right ?]}
...
\aim{[Rahul: We do not want  to refer to this code this way. A better way would be to have a specific release, put it on zenodo, and then reference the doi in the references. That is reproducible.]}
