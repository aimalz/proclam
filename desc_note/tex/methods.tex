...
Traditional classification metrics may be modified for evaluation on probabilistic classifications (Lochner+16, M\"{o}ller+16, Hon,Stello,Zinn18) but only by reducing class probabilities to point estimates of class.
For the purposes of making decisions about the allocation of limited spectroscopic follow-up resources, reduction to point estimates is appropriate, because the action of making a spectroscopic observation is itself deterministic.
The landscape for population-level inference in astronomy is changing 
...
The Brier score (Brier50) is a mean square error calculated between the true class indicator vector and the estimated probability vector.
