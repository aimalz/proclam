...
A CPM with uniform probabilities for all classes, which we denote with $\mathbb{U}$) as shown in the leftmost top panel of Figure~\ref{fig:mock_cm}, would correspond to uniform random guesses for deterministic classification, but in accordance with Equation~\ref{eq:cmtoprob}, the classification posteriors are perturbations away from a uniform distribution across all classes.
