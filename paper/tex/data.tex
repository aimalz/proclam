...
Using the above, we can construct classification posteriors $p(\hat{m} \mid m')$ which we will use to make example `draws' of classificaation probability submissions. We refer to the matrix $\mathbb{C}$ comprised of $p(\hat{m} \mid m')$ as the \textit{conditional probability matrix} (CPM).
