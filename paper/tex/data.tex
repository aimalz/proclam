...
\textbf{RH: Note that I made the above change because the section below is an extremely confusing and symbolic-heavy way to explain draws from the probability distribution - this was changed in response to multiple reviewer comments. If you don't like my suggestion above please make a different change - but the paragraph below is too confusing}
...
In order to observe metric performance on different classification schemes, we simulate some archetypical mock classifiers, devised to produce generic responses to a classification challenge, without any interaction with actual challenge data.
...
Using the above, we can construct classification posteriors $p(\hat{m} \mid m')$ which we will use to make example `draws' of classificaation probability submissions. We refer to the matrix $\mathbb{C}$ composed of $p(\hat{m} \mid m')$ as the \textit{conditional probability matrix} (CPM).

% Removed the paragraph below since we really want to keep this simple and non-confusing - and isn't used again.
...
Assuming the light curves contain information about the true class (an assumption that underlies classification as a whole), we can use the appropriate row $\mathbb{C}_{m'_{n}} = p(\hat{m} \mid m', \mathcal{C})$ of the CPM $\mathbb{C}$ as a proxy for $p(m \mid d_{n}, D, \mathcal{C})$, without directly classifying light curves themselves\footnote{This was important given that this paper should be general and not contain any knowledge of the \plasticc\ dataset simulation procedure.}. To emulate the effect of natural variation of information content in different light curves using the above, we generate a posterior probability vector $\vec{p}(m \mid m', \mathbb{C})$ by taking a Dirichlet-distributed draw

%we generate a posterior probability vector $\mathbf{p}$, with elements $p_m \equiv p(m | m', \mathbb{C})$, by taking a draw from a Dirichlet distribution:
%\begin{equation}
%\mathbf{p} | d_n, D, \mathcal{C} \sim \text{Dir}[\mathcal{C}_{m'_n} \delta]
%\end{equation}
...
%Without loss of generality, we can decompose $\mathbb{C}$ under some basis functions of parameters $\mathcal{C}$, the same as those introduced above in the definition of a classification posterior $p(m \mid d_{n}, D, \mathcal{C})$.
%The CPM $\mathbb{C}$ thus defines the behavior of a classifier.
%Since we believe the light curves contain information about the true class (an assumption that underlies classification as a whole), we can use the appropriate row $\mathbb{C}_{m'_{n}} = p(\hat{m} \mid m', \mathcal{C})$ of the CPM $\mathbb{C}$ as a proxy for $p(m \mid d_{n}, D, \mathcal{C})$, without directly classifying light curves themselves.\footnote{The introduction of the CPM permits this study to be conducted without any knowledge of the \plasticc\ dataset's simulation nor validation.}
...
%		\caption{A realistically complex conditional probability matrix and classification posteriors drawn from the probability matrix. The top row shows the conditional probability matrix which contains a combination of the various individual systematics describes in this section, each systematic acting on a subset of the classes. To illustrate what the output of a particular classifier will be, in the bottom row we draw a classification posterior from the conditional probability matrix above, for 27 randomly simulated objects. For each row we indicate the true class with a red star (and indicate the class number on the left of the probability matrix). 
...
An actual classifier should be more complex than the simplified cases of Figure~\ref{fig:mock_cm}, with different behavior for each class.
To demonstrate the procedure by which mock classification posteriors are generated from rows of the CPM, 
we provide 22 examples of draws of the posterior CPM in the bottom panel of Figure~\ref{fig:mock_probs}.
...
A CPM with uniform probabilities for all classes (which we will denote with $\mathbb{U}$) as shown in the leftmost top panel of Figure~\ref{fig:mock_cm}, would correspond to uniform random guesses for deterministic classification, but as highlighted in Equation~\ref{eq:cmtoprob}, the classification posteriors are perturbations away from a uniform distribution across all classes.
...
We can consider the \textit{uncertain} classifier as an experimental control for the least efficient possible classification scheme, bearing in mind that if classifications were anticorrelated with true classes, the experimenter could simply reassign the classification labels to improve performance under any metric.

To construct a final CPM we:

\begin{itemize}
\item combine the eight classifiers over (assigning a randomly drawn `type' of classifier for each class) to obtain a total CPM
\item draw from this random CPM to obtain an example classification row for each object in the sample
\end{itemize}

An example of one such combined CPM is shown in Figure~\ref{fig:mock_probs}, along with examples of the mock classification posteriors drawn from it for a range of objects. 
...
In addition to a perfect classifier, we test linear combinations of the perfect and uncertain CPMs ($\mathbb{C} = (s + 1)^{-1} \left(s\mathbb{I} + \mathbb{U}\right)$) where the contribution of the perfect classifier is greater than that of the uncertain classifier by a factor of $s > 0$.
...
For a single science application, however the goal of \plasticc\ is to serve the needs of those who study a wide variety of classes for different purposes.
...
Class $m'$ is said to be subsumed (or absorbed) by class $\tilde{m}$ A \textit{subsuming} classifier absorbs or subsumed class $m'$ into class $\tilde{m}$, as shown in the right-central bottom panel of Figure~\ref{fig:mock_cm}. The subsuming classifier may be asymmetric, or the classes may be mutually subsumed (rightmost bottom panel of Figure~\ref{fig:mock_cm}) if one already has significant off-diagonal probability, as is true for the uncertain classifier.
...
Tt is more common for SN Ibc to be misclassified as SN Ia than the other way around.
...
An extreme case of inaccurate classification is to classify all objects as the most common class (in the training or test set), which is of particular concern given non-representative training data that contains class imbalance. Such a \textit{cruise control} classifier (left-center bottom panel of Figure~\ref{fig:mock_cm}) counters \plasticc's goal of identifying objects belonging to extremely rare classes.
...
\begin{figure*}
    ...
    Bottom row: the same five machine learning methods applied to wavelet decompositions as features.
		These CPMs derived from the dataset of a precursor light-curve classification challenge by modern methods exhibit some of the systematics identified in Section~\ref{sec:mockdata} and Figure~\ref{fig:mock_cm}, particularly cruise control (WKNN, WNB), noisy (class Ibc in all but TBRF and WKNN), and perfect (class II in all). It is worth noting that the \citet{lochner_photometric_2016} applied their classification to a representative sub-sample of the \snphotcc\ data selected once the challenge was complete, and so some of the issues of non-representativity present in the original submissions to the \snphotcc\ were not replicated.\label{fig:snphotcc_cm}}
	\end{center}
\end{figure*}
...
The \snphotcc\ attracted diverse classification approaches, encompassing $\chi^{2}$ fits of the supernova light curves to publicly available templates \citep{nugent_kcorrections_2002}, empirical models \citep{conley_sifto:_2008}, as well as alternatives to curve-fitting such as outlier identification on the training set Hubble diagram\footnote{we give an introduction to both the \plasticc\ data and also the background introduction to the Hubble diagram of distance vs. redshift and its usefulness to cosmology in the \plasticc\ release note: \href{arXiv:XXXX.XXXXX}{arXiv:XXXX.XXXXX}}, dimensionality reduction, and clustering.
...
We consider a collection of probabilistic classification methods, as presented in \citet{lochner_photometric_2016}, whose CPMs\footnote{The classifiers of \citet{lochner_photometric_2016} are indeed probabilistic but are reduced to confusion matrices via deterministic labels (by assigning the class label according to the highest probability) for this visualization and the science-motivated metric of Section~\ref{sec:deterministic}.  In all other instances, the classification posteriors are used directly.} are shown in Figure~\ref{fig:snphotcc_cm}.
The set of classification algorithms includes template-based classification procedures, denoted as T, (\citet{sako_photometric_2011}, top row) and a wavelet decomposition, denoted as W, of the light curves to construct the features over which to classify (\citet{newling_statistical_2011}, bottom row), each paired with Boosted Random Forest (BRF), K-Nearest Neighbors (KNN), Naive Bayes (NB), Neural Network (NN), and Support Vector Machine (SVM) machine learning algorithms (columns). While the complexity of entries to the \snphotcc\ was greater than this subset, we use these examples to establish the behavior of our metrics on realistic classification submissions.
...
Note that the WNN and WNB methods both suffer from the cruise control systematic on SN II, which were the most prevalent in the \snphotcc\ dataset. Nearly all the other CPMs exhibit classifications that are almost perfect for SN Ia, perfect for SN II, and noisy for SN Ibc.
