...
\begin{figure}
    ...
    \includegraphics[width=0.49\textwidth]{./fig/examples.png}
		...
		Top panel: An example of a realistically complex conditional probability matrix, constructed by selecting a systematic for each individual class.
		This illustrates (for example), how a classifier may confuse some classes with high confidence and may be uncertain about others, whilst still classifying still other classes perfectly.
		Bottom panel: Example classification probabilities, drawn from the above conditional probability matrix, with their true class indicated by a red star and the systematic affecting that true class described on the right.
		This simulates how a set of objects with different true classes would be classified by the classifier with behaviour characterised by the above conditional probability matrix.
		One can see that classification posteriors that share a true class, and thus a generating row of the conditional probability matrix, still take different values from one another, due to the Dirichlet process, which emulates the impact of the variation expected among different lightcurves within the same class.
		}
