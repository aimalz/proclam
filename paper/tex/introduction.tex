...
With its rapid scan strategy, exquisite depth, and many photometric filters, \lsst\ will deliver millions of multi-band time-series, generically called lightcurves, enabling unprecedented population-level studies of time-varying astrophysical sources.
...
However, the volume of objects anticipated of \lsst, as well as their potentially low signal-to-noise ratios, likely exceeds the availability of spectroscopic follow-up resources; the great majority of \lsst's time-varying discoveries will never be spectroscopically confirmed.
...
As an example, an object with probability $0.9$ of being in a certain class $m$ might be assigned a deterministic classification of being of class $m$, effectively rounting up the probability to $1$, yielding a point estimate of $m$ for that object.
