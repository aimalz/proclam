...
In Section~\ref{sec:past}, we review the metrics that were used in \snphotcc\ as the most similar previous challenge.
...
\begin{eqnarray}
  ...
  \mathcal{C}_{FOM-Ia} &\equiv& \epsilon_{Ia} \times PP_{Ia}
  \frac{1}{\mathcal{N}_{Ia}^{TOT}}\times \frac{(N_{Ia}^{\mathrm{true}})^2}{N_{Ia}^\mathrm{true}+W_{Ia}^\mathrm{false}N_{Ia}^\mathrm{false}}
\end{eqnarray}
of the ratio of the \textit{efficiency}
\begin{eqnarray}
  \label{eq:efficiency}
  \epsilon_{Ia} &=& \frac{1}{\mathcal{N}_{Ia}^{TOT}}
\end{eqnarray}
of SN Ia classification and \textit{pseudo-purity}
\begin{eqnarray}
  \label{eq:pseudopurity}
  PP_{Ia} &=& \frac{(N_{Ia}^{\mathrm{true}})^2} {N_{Ia}^\mathrm{true} + W_{Ia}^\mathrm{false}N_{Ia}^\mathrm{false}}
\end{eqnarray}
with an additive penalty term with weight $W_{Ia}^\mathrm{false}$ for false positives, motivated by the expense of squandering SN Ia follow-up resources on an object of a different class.
...
Equation~\ref{eq:snphotccfom} reduces to $\mathcal{C}_{FOM-Ia}  = \epsilon_{Ia} + PP_{Ia},$ the efficiency and pseudopurity, which can be interpreted as the traditional purity factor in the limit that the weight $W_{Ia}^\mathrm{false} = 1$.
