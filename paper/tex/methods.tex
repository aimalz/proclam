...
We thus agnostically test weighting schemes\footnote{\changes{The weights considered in this study are more extreme than those ultimately used for \plasticc\ because the true weights were withheld from some authors prior to the end of the challenge.
However, in the Kaggle framework, it is possible to estimate these values by systematically probing the output of the public leader board with entries from the cruise control classifier archetype targeting each class one at a time.
Some \plasticc\ competitors did, in fact, execute this procedure and publicly announced the weights they had discovered, making the information available to all participants.}}
