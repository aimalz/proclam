...
% 	\label{fig:cruise}
% \end{figure}
In Figure~\ref{fig:all_combined}, we demonstrate the behavior of the metrics with conditional probability matrices composed of pairs of baseline classifier error properties.
...
\begin{figure}
	\begin{center}
		...
		The $x$-axis is the Brier score and the $y$-axis is the log-loss. corresponds to an almost perfect classifier that with one class instead affected by a systematic (shape), with log-loss on the $y$ axis and Brier score on the $x$ axis.
		}
	\end{center}
...
We also vary the weights placed on the class affected by the worsening systematic and the integrated weight of all other classes with baseline behavior.
We observe linear relationships between the log-loss and Brier score metrics for each pair of baseline and systematic as the weights are varied.
Though the linearity is not surprising, there is information to be gleaned from the slopes.

\aim{Still propagating changes below here!}
